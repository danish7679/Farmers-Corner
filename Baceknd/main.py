...
async def register(user: User = Body(...)):
    ...
    try:
        ...
        dup_user =await collection.find_one({"email": user.email})
...
async def login(user_credentials: UserLogin):
    ...
    if user_credentials.email=="danishpvt23@gmail.com":
...
async def scan_crop(data:getcrop):
    try:
        ...
        model_path=r"C:\model\resnet_15confusionepoch_model.pth"
        ...
        print(class_id)
        if predicted_class_name != "No Class Found":
